...
public class RemoveNameFromFunctionExpressionFix(element: JetNamedFunction) : JetIntentionAction<JetNamedFunction>(element) {
    ...
    companion object : JetSingleIntentionActionFactory() {
        ...
        private fun isFunctionExpression(function: JetNamedFunction): Boolean {
            ...
            return !(parent is JetBlockExpression || parent is JetClassBody || parent is JetFile)
        }
        ...
        private fun removeNameFromFunction(function: JetNamedFunction) {
            ...
            // remove whitespace after 'fun' if next sibling is '()' (may be extension type reference)
            if (function.getFunKeyword()?.getNextSiblingIgnoringWhitespaceAndComments()?.getNode()?.getElementType() ==
                    JetNodeTypes.VALUE_PARAMETER_LIST) {
                (function.getFunKeyword()?.getNextSibling() as? PsiWhiteSpace)?.delete()
            }

            if (wereAutoLabelUsages) {
                val psiFactory = JetPsiFactory(function)
                val newFunction = psiFactory.createExpressionByPattern("$name@ $0", function.getText())
