...
private class RoundEnvironmentWrapper(
    ...
    override fun getElementsAnnotatedWith(a: TypeElement): MutableSet<out Element>? {
        val elements = parent.getElementsAnnotatedWith(a).filter { !it.isIgnoredType() }.toHashSet()
    ...
    private fun Element.isIgnoredType() = when (this) {
        is TypeElement -> "__gen.annotation.Cl" == getQualifiedName().toString()
        else -> false
    }
}
