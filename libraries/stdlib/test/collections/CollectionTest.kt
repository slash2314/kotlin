...
class CollectionTest {
    ...
    private fun <T> CompareContext<Set<T>>.setBehavior() {
        equalityBehavior()
        collectionBehavior()
    ...
    private fun <K, V> CompareContext<Map<K, V>>.mapBehavior() {
        ...
        compareProperty( { keySet() }, { setBehavior() } )
        compareProperty( { entrySet() }, { setBehavior() } )
        compareProperty( { values() }, { collectionBehavior() })
    }
    ...
    private fun <T> CompareContext<Collection<T>>.collectionBehavior() {
        propertyEquals { size() }
        propertyEquals { isEmpty() }
